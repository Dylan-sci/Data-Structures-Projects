...
/***------------------  BEGIN AUGEMENTED METHOD ----------------***/
   ...
   public boolean isPath(E fromKey, E toKey)
   {
       ...
       Vertex tmp = first;
       while (fromKey.compareTo(tmp.data) != 0 && tmp != null)
       {
           tmp = tmp.pNextVertex;
       }
       if (tmp == null)
       ...
       LinkedList<Vertex> t = new LinkedList<>();
       t.add(tmp);
       
       while (!t.isEmpty())
       {
           tmp = t.remove(0);
           if (tmp.data.compareTo(toKey) == 0)
           {
               return true;
           }
            s.add(tmp);
            ...
            while (edge1 != null)
            {
                if (!s.contains(edge1.destination))
                {
                    t.add(edge1.destination);
                }
                edge1 = edge1.pNextEdge;
            }
       }
       return false;
   ...
   {
       long outDegree = first.outDeg;
       Vertex tmp = first.pNextVertex;
       while (tmp != null)
       {
           outDegree += tmp.outDeg;
           tmp = tmp.pNextVertex;
       }
       return outDegree;
      //implement this method
